...
export function CurrentCondition(props: any) {
  ...
  const precipitation = isMetric
    ? current.precipitation.metric
    : current.precipitation.imperial
  const temperature = isMetric
    ? current.temperature.metric
    : current.temperature.imperial
  const { direction } = current.wind
  const speed = isMetric
    ? current.wind.speed.metric
    : current.wind.speed.imperial
    ...
    <>
      <div className="flex flex-row my-8">
        ...
        <div className="w-1/2 px-4">
          ...
          <div className={css}>
            Wind: {direction.locolized} {speed.value} {speed.unit}
          </div>
        </div>
      </div>
    </>
