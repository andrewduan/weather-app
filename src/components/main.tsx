    ...
    useEffect(() => {
      if (debouncedLocation) {
        ...
        const fetchWeather = async () => {
          const currentWeather = await fetch(`${baseUrl}/currentconditions/v1/${locationId}?apikey=${apiKey}&details=true`);
          const forecasts =  await fetch(`${baseUrl}/forecasts/v1/daily/5day/${locationId}?apikey=${apiKey}&details=true&metric=${isMetric}`);
